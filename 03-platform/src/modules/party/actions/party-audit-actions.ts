 ...
 * Expose Party Audit History server actions to the App Router UI.
...
import type { PartyAuditListFiltersInput } from "@/modules/party/validators/party-audit-validators";
...
export async function listPartyAuditHistoryAction(

  partyId: string,

  filters?: PartyAuditListFiltersInput

): Promise<AuthActionResult<PartyAuditHistoryPanelView>> {

  try {

    const context = await requirePartyContext();

    const service = createPartyAuditQueryService();

    const data = await service.getAuditPanel(context, partyId, filters);
...
export async function loadMorePartyAuditHistoryAction(
...
export async function getPartyAuditDetailAction(
  ...
  try {
    ...
    const data = await service.getAuditDetail(context, partyId, auditId);
